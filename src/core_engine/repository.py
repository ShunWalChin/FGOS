...
async def claim_next_social_post(
    ...
    *,
    ...
    cooldown (docs/ARCHITECTURE.md §6). `FOR UPDATE OF pq SKIP LOCKED` keeps two
    workers from grabbing the same post. The account token is decrypted (pgcrypto)
    and returned so the worker never reads ciphertext.
    """
...
async def mark_social_post_failed(
    ...
    *,
    ...
    (e.g. an invalid payload that retrying will never fix)."""
